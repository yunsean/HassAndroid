...
class MainFragment : BaseFragment() {
    ...
    private fun ui() {
        this.local.setText(ctx.readPref("qtfm.provinceName", "北京", "DEFAULT"))
        this.category.setText(ctx.readPref("qtfm.categoryName", "音乐台", "DEFAULT"))
                ...
                qtfmApi.getCategories()
                        .nextOnMain {
                            ...
                            provinceList.adapter = RecyclerAdapter(R.layout.listitem_hass_broadcast_filter, it.data?.regions_map) {
                                view, index, province ->
                                view.name.text = province.name
                                view.onClick {
                                    switchPanel(provincePanel, categoryPanel, localDropdown, categoryDropdown, true)
                                    ctx.savePref(arrayOf("qtfm.provinceCode", "qtfm.provinceName"), arrayOf(province.id, province.name))
                                    RxBus2.getDefault().post(XmlyFilterChange(XmlyFilterChange.Companion.FilterMode.province, province.id))
                                    fragment.local.text = province.name
                                }
                            }
                        ...
                        .nextOnMain {
                            ...
                            categoryList.adapter = RecyclerAdapter(R.layout.listitem_hass_broadcast_filter, it.data?.channel_categories) {
                                ...
                                view.onClick {
                                    ...
                                    ctx.savePref(arrayOf("qtfm.categoryCode", "qtfm.categoryName"), arrayOf(category.id, category.name))
