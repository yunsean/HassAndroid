...
class PinchImageView : android.support.v7.widget.AppCompatImageView {
    ...
    var pinchMode = PINCH_MODE_FREE
        private set
    val mask: RectF?
        get() = if (mMask != null) RectF(mMask) else null
    ...
    fun getOuterMatrix(matrix: Matrix?): Matrix {
        var matrix = matrix
        if (matrix == null) matrix = Matrix(mOuterMatrix)
        else matrix.set(mOuterMatrix)
        return matrix
    }
    ...
    private inner class MaskAnimator(start: RectF, end: RectF, duration: Long) : ValueAnimator(), ValueAnimator.AnimatorUpdateListener {
    ...
    private inner class FlingAnimator(vectorX: Float, vectorY: Float) : ValueAnimator(), ValueAnimator.AnimatorUpdateListener {
    ...
    private inner class ScaleAnimator @JvmOverloads constructor(start: Matrix, end: Matrix, duration: Long = SCALE_ANIMATOR_DURATION.toLong()) : ValueAnimator(), ValueAnimator.AnimatorUpdateListener {
    ...
    private abstract class ObjectsPool<T> (private val mSize: Int) {
        private val mQueue: Queue<T>
        init {
            mQueue = LinkedList()
        }
        fun take(): T {
            return if (mQueue.size == 0) newInstance() else resetInstance(mQueue.poll())
        }
        fun given(obj: T?) {
