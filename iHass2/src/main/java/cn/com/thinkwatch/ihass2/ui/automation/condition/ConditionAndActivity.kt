...
class ConditionAndActivity : BaseActivity() {

    private lateinit var condition: AndCondition
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_hass_automation_condition_and)
        setTitle("全部满足条件", true, "保存")
        setAutoHideSoftInput(AutoHideSoftInputMode.WhenClick)

        val condition = intent.getStringExtra("condition")
        if (condition == null) this.condition = AndCondition()
        else this.condition = Gsons.gson.fromJson(condition, AndCondition::class.java)
    ...
    override fun doRight() {
        ...
        val condition = AndCondition(conditions)
